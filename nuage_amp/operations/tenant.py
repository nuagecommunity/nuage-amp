...
def keystone_tenant_exists(tenant_name):
    ...
    try:
        if ks.tenants.find(name="{0}".format(tenant_name)):
            return True
        else:
            return False
    except:
...
def delete_vms_in_tenant(tenant_name):
    logger.info("Deleting all VMs for tenant: {0}".format(tenant_name))
    nova_creds = get_admin_nova_creds(tenant_name)
    nova = novaclient.Client(**nova_creds)
    try:
        for server in nova.servers.list():
            logger.info("Deleting Server: {0}".format(server['id']))
            server.delete()
    except Exception, e:
        logger.error("|- ERROR deleting VM: {0}".format(server['id']))
        logger.error(repr(e))
    return
...
def delete_vsdobjects_in_tenant(tenant_name):
    ...
    enterprise = nc.get("enterprises", filtertext="name == \"{0}\"".format(tenant_name)).obj()[0]
    ...
    try:
        ...
        for domain in domains:
            nc.put("domains/{0}".format(domain["ID"]), {"maintenanceMode": "ENABLED"})
            ...
            for vport in vports:
                ...
                for alarm in alarms:
                    try:
                        ...
                        logger.error(repr(e))
                nc.delete("vports/{0}".format(vport["ID"]))
            nc.delete("l2domains/{0}".format(domain["ID"]))
    except Exception, e:
...
def create_vsd_managed_tenant(tenant_name):
    logger.info("Creating VSD Managed Tenant: {0}".format(tenant_name))
    creds = get_keystone_creds()
    keystone = ksclient.Client(**creds)
    logger.info("Creating Keystone Tenant: {0}".format(tenant_name))
    try:
    ...
    neutron = neutronclient.Client(**neutron_creds)
    try:
        logger.info("Creating Net-Partition: {0}".format(tenant_name))
        body_netpart = {
            "net_partition":
                {
                    "name": tenant_name
        ...
        logger.error("|- ERROR creating netpartition: {0}".format(tenant_name))
        logger.error(repr(e))
    logger.info("Finished Creating VSD Managed Tenant: {0}".format(tenant_name))


def delete_vsd_managed_tenant(tenant_name, force):
    logger.info("Deleting VSD Managed Tenant: {0}".format(tenant_name))
    creds = get_keystone_creds()
    keystone = ksclient.Client(**creds)
    if force:
        try:
            ...
            logger.error("|- ERROR deleting VMs from tenant {0}".format(tenant_name))
            logger.error(repr(e))
        try:
            delete_vsdobjects_in_tenant(tenant_name)
        except Exception, e:
            logger.error("|- ERROR deleting VSD Objects from tenant {0}".format(tenant_name))
            logger.error(repr(e))
    if keystone_tenant_exists(tenant_name):
        try:
            logger.info("Deleting Keystone Tenant: {0}".format(tenant_name))
            os_tenant = keystone.tenants.find(name="{0}".format(tenant_name))
            os_tenant.delete()
        except Exception, e:
            logger.error("|- ERROR deleting tenant {0} in keystone".format(tenant_name))
            logger.error(repr(e))
            return 1
    else:
        logger.info("Keystone tenant {0} not present".format(tenant_name))
    neutron_creds = get_neutron_creds(cfg.get('openstack', 'admin_username'), cfg.get('openstack', 'admin_password'),
                                      "admin")
    neutron = neutronclient.Client(**neutron_creds)
    try:
        logger.info("Deleting Net-Partition: {0}".format(tenant_name))
        if not netpartition_exists(tenant_name):
            logger.error("|- Netpartition: {0} not found. Already deleted?".format(tenant_name))
            return
        netpart = neutron.list_net_partitions(name=tenant_name)['net_partitions'][0]
        neutron.delete_net_partition(netpart['id'])
    except Exception, e:
        logger.error("|- ERROR deleting netpartition: {0}".format(tenant_name))
        logger.error(repr(e))
    logger.info("Finished Deleting VSD Managed Tenant: {0}".format(tenant_name))
